...
::ExtraItems <- {
	"f" : {
		...
		"fire particle red": "drg_phlo_stream"
	}
}
IncludeScript("popextensions/item_map.nut", getroottable())
...
::CustomWeapons <- {
	...
	SpawnHookTable = {
		function RegenerateCustomWeapons(params) {
			...
			if (("ExtraLoadout" in player.GetScriptScope())) {
				...
				NetProps.SetPropIntArray(player, "m_iAmmo", CustomWeapons.GetMaxAmmo(player, 1), 1)
				NetProps.SetPropIntArray(player, "m_iAmmo", CustomWeapons.GetMaxAmmo(player, 2), 2)
				NetProps.SetPropIntArray(player, "m_iAmmo", CustomWeapons.GetMaxAmmo(player, 3), 3)
				NetProps.SetPropIntArray(player, "m_iAmmo", CustomWeapons.GetMaxAmmo(player, 4), 4)
			}
		}
	}
	...
	function GiveItem(itemname, player)
	{
		...
		local playerclass = CustomWeapons.Classes[player.GetPlayerClass()]
		...
		if (item_slot == "primary")
		{
			if (this.TF_AMMO_PER_CLASS_PRIMARY[playerclass] != this.TF_AMMO_PER_CLASS_PRIMARY[animset])
			{
				...
				{
					local ammofix = Entities.CreateByClassname("tf_wearable")
					NetProps.SetPropBool(ammofix, "m_bValidatedAttachedEntity", true)
					NetProps.SetPropBool(ammofix, "m_AttributeManager.m_Item.m_bInitialized", true)
					NetProps.SetPropEntity(ammofix, "m_hOwnerEntity", player)
				...
				NetProps.SetPropIntArray(player, "m_iAmmo", GetMaxAmmo(player, 1), 1)
			}
		}
		...
		if (item_slot == "secondary")
		{
			if (this.TF_AMMO_PER_CLASS_SECONDARY[playerclass] != this.TF_AMMO_PER_CLASS_SECONDARY[animset])
			{
				...
				player.GetScriptScope().CustomWeapons.ammofix.ReapplyProvision()
				NetProps.SetPropIntArray(player, "m_iAmmo", GetMaxAmmo(player, 2), 2)
			}
		}
		...
		if (slot != null)
		{
			...
			NetProps.SetPropInt(item, "m_iViewModelIndex", PrecacheModel(armmodel))

			// worldmodel
			player.GetScriptScope().thinkTable.CustomWeaponWorldmodel <- UpdateWorldmodel
			...
			// viewmodel arms
			NetProps.SetPropInt(item, "m_nRenderMode", kRenderTransColor)
		...
		if ("CustomWeapons" in self.GetScriptScope())
			{
				...
				{
					if (("worldmodel" in self.GetScriptScope().CustomWeapons) && (NetProps.GetPropInt(self.GetScriptScope().CustomWeapons.worldmodel, "m_nModelIndex") != self.GetScriptScope().CustomWeapons[activeweapon]))
					{
						...
						local tpWearable = Entities.CreateByClassname("tf_wearable")
						NetProps.SetPropInt(tpWearable, "m_iTeamNum", self.GetTeam())
						NetProps.SetPropInt(tpWearable, "m_nModelIndex", modelindex)
						NetProps.SetPropBool(tpWearable, "m_bValidatedAttachedEntity", true)
						NetProps.SetPropBool(tpWearable, "m_AttributeManager.m_Item.m_bInitialized", true)
						NetProps.SetPropEntity(tpWearable, "m_hOwnerEntity", self)
	...
	function GetMaxAmmo(player, slot)
	{
		...
		return multiplier * slottable[CustomWeapons.Classes[player.GetPlayerClass()]]
	}
}
