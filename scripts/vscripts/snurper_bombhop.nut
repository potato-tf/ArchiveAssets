...
if (!("hoprange" in self.GetScriptScope()))	// check for a unique variable that no other script uses to avoid compatibility problems
{
	...
	hoprange <- 400.0							// the roughly maximum distance in HUs that the bomb will aim to travel on each hop (set to 0 for the bomb to hop in place)
	hoptime_min <- 8						// shortest possible cooldown between bomb hops
	hoptime_max <- 10 						// longest possible cooldown between bomb hops
	...
	evaluatedisabledprefers <- false			// should path calculation take disabled func_nav_prefer entities into consideration for better path determination? (this may produce path failures on sequoia-styled gate maps that use prefers for guiding bots through shortcuts)
